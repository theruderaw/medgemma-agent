...
@router.get("/v1/config", response_model=AppConfigResponse)
...
@router.get("/v1/addons", response_model=AddonListResponse)
async def list_addons(session_id: str | None = None) -> AddonListResponse:
    """All registered add-ons with their per-session toggle state.

    ``enabled`` reflects the session's stored overrides (a missing row means
    enabled); without ``session_id`` every addon reports its default state.
    """
    addons = all_addons()
    disabled = (
        await get_settings_store().get_disabled_addon_names(session_id)
        if session_id
        else set()
    )
...
@router.post("/v1/addons/{name}", response_model=AddonInfo)
async def toggle_addon(name: str, body: AddonToggleRequest, session_id: str) -> AddonInfo:
    """Enable or disable one add-on for this session.

    Toggles are session-scoped; the emergency floor is not an add-on and can
    never be toggled. Returns 404 for an unknown addon or session.
    """
    ...
    await get_settings_store().set_addon_enabled(session_id, name, body.enabled)
    await audit.append(
        module="addons",
        event_type="addon_toggled",
        payload={"addon": name, "enabled": body.enabled},
        session_id=session_id,
    )
    logger.info("addons.toggled", addon=name, enabled=body.enabled, session_id=session_id)
    return AddonInfo(
        name=name,
        description=addon.tool_schema.description,
        enabled=body.enabled,
        disclaimer_level=addon.safety_profile.disclaimer_level,
    )
...
@router.get("/v1/sessions/{session_id}/messages", response_model=SessionHistoryResponse)
