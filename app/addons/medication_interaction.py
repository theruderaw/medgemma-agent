...
Unlike the other addons, the interaction claim itself NEVER originates
from a model: the pair is looked up in the curated dataset
(``app/addons/data/drug_interactions.json``) and any model output is only
...
class MedicationInteractionAddon:
    ...
    system_prompt = load_prompt("medication_extraction")
...
addon = MedicationInteractionAddon()
