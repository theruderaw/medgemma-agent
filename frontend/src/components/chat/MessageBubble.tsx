...
function ThinkingRow({ text }: { text: string }) {
    return (
        <div className="flex animate-rise items-center gap-2 self-start py-1 pl-1 text-base italic text-neutral-500">
...
export default function MessageBubble({ message }: { message: ChatMessage }) {
    ...
    return (
        <div
            className={`flex max-w-[85%] animate-rise flex-col break-words leading-relaxed ${alignment} rounded-xl border px-4 py-3 ${
                tone
            } ${message.role === 'error' ? 'max-w-[90%] self-center' : ''}`}
        >
            <span className="mb-1 flex items-center gap-2 text-xs uppercase tracking-wider text-neutral-500">
            ...
            {message.imagePreview &&
                (message.imagePreview.startsWith('data:application/pdf') ? (
                    <div className="mb-2 flex items-center gap-2 self-start rounded-lg border border-neutral-300 bg-neutral-100 px-2.5 py-1.5 text-sm text-neutral-700 dark:border-neutral-700 dark:bg-neutral-800 dark:text-neutral-300">
                        <FileText className="h-4 w-4" aria-hidden="true" /> PDF attached
                    </div>
                ) : (
                    <img
                ...
                <>
                    ...
                    {message.specialistNote != null && (
                        <button
                            ...
                            className="mb-2 flex cursor-pointer items-center gap-1.5 self-start rounded-full border border-neutral-400 bg-neutral-100 px-2.5 py-1 text-xs uppercase tracking-wider text-neutral-700 transition-colors hover:border-neutral-600 hover:text-neutral-950 dark:border-neutral-600 dark:bg-neutral-900 dark:text-neutral-200 dark:hover:border-neutral-400 dark:hover:text-neutral-50"
                        >
                            Clinical note
            ...
            <MessageContent text={message.text} streaming={message.streaming} />
