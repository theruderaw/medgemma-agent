...
export default function SlashMenu({ matches, activeIndex, onPick, onHover }: Props) {
  return (
    ...
    >
      <p className="m-0 border-b border-neutral-100 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-neutral-400 dark:border-neutral-800 dark:text-neutral-500">
      ...
      {matches.map((addon, i) => (
        ...
        >
          ...
          <span className="line-clamp-1 text-xs text-neutral-500 dark:text-neutral-400" title={addon.description}>
            {addon.description}
          </span>
        </button>
      ))}
