...
with an append-only audit trail, and optional Celery-backed worker queues that
move inference off the HTTP request/response cycle.

**Milestone 6.2** — Worker queues: turn processing can run in a dedicated Celery
worker (queued mode) instead of inline in the request handler, while keeping
sync mode as the default.
...
```
app/
  main.py            # FastAPI endpoints + error mapping + alembic on startup
  ...
  services/
    chat.py          # run_chat_turn() + run_emergency_turn() orchestration
